...
namespace Kingdoms.Bot.UI
{
    ...
    internal class CopyPopularitySettingsForm : Form
    {
        private static readonly Color FormBg = Color.FromArgb(28, 30, 38);
        private static readonly Color InputBg = Color.FromArgb(50, 52, 64);
        private static readonly Color TextPri = Color.FromArgb(230, 230, 240);
        ...
        public CopyPopularitySettingsForm()
        {
            _copied = false;
            this.Text = "Copy Popularity Settings";
            this.BackColor = FormBg;
            this.ForeColor = TextPri;
            this.Font = new Font("Segoe UI", 9f);
            this.ClientSize = new Size(440, 400);
        ...
        private void PopulateVillages()
        {
            ...
            {
                int id = v.villageID;
                if (GameEngine.Instance.World.isCapital(id)) continue;
                string name = GameEngine.Instance.World.getVillageName(id);
                PpVillageItem item = new PpVillageItem(id, "[" + id + "] " + name);
                _sourceCombo.Items.Add(item);
                _targetList.Items.Add(item);
            }
        ...
        private void DoCopy()
        {
            PpVillageItem sourceItem = _sourceCombo.SelectedItem as PpVillageItem;
            if (sourceItem == null) { ShowStatus("Select a source village.", true); return; }
            ...
            for (int i = 0; i < _targetList.Items.Count; i++)
            {
                if (_targetList.GetItemChecked(i))
                {
                    PpVillageItem item = (PpVillageItem)_targetList.Items[i];
                    if (item.VillageId != sourceItem.VillageId)
                        targetIds.Add(item.VillageId);
                }
            }

            if (targetIds.Count == 0) { ShowStatus("Select at least one target village.", true); return; }
            ...
            VillagePopularitySettings source = s.GetVillageSettings(sourceItem.VillageId);

            int count = 0;
            foreach (int targetId in targetIds)
            {
                VillagePopularitySettings target = s.GetVillageSettings(targetId);
            ...
            _copied = true;
            ShowStatus("Copied settings to " + count + " village(s).", false);
            BotLogger.Log("Popularity", BotLogLevel.Info,
                "Copied popularity settings from " + sourceItem.Display + " to " + count + " village(s).");
        }
        ...
        private class PpVillageItem
        {
            public int VillageId;
            public string Display;
            public PpVillageItem(int id, string display) { VillageId = id; Display = display; }
            public override string ToString() { return Display; }
        }
    }

    internal class CopyBanquetSettingsForm : Form
    {
        private static readonly Color FormBg = Color.FromArgb(28, 30, 38);
        private static readonly Color InputBg = Color.FromArgb(50, 52, 64);
        private static readonly Color TextPri = Color.FromArgb(230, 230, 240);
        ...
        public CopyBanquetSettingsForm()
        {
            _copied = false;
            this.Text = "Copy Banquet Settings";
            this.BackColor = FormBg;
            this.ForeColor = TextPri;
            this.Font = new Font("Segoe UI", 9f);
            this.ClientSize = new Size(440, 400);
        ...
        private void PopulateVillages()
        {
            ...
            {
                int id = v.villageID;
                // Banquets only apply to regular villages
                if (GameEngine.Instance.World.isCapital(id)) continue;
                string name = GameEngine.Instance.World.getVillageName(id);
                BqVillageItem item = new BqVillageItem(id, "[" + id + "] " + name);
                _sourceCombo.Items.Add(item);
                _targetList.Items.Add(item);
            }
        ...
        private void DoCopy()
        {
            BqVillageItem sourceItem = _sourceCombo.SelectedItem as BqVillageItem;
            if (sourceItem == null) { ShowStatus("Select a source village.", true); return; }
            ...
            for (int i = 0; i < _targetList.Items.Count; i++)
            {
                if (_targetList.GetItemChecked(i))
                {
                    BqVillageItem item = (BqVillageItem)_targetList.Items[i];
                    if (item.VillageId != sourceItem.VillageId)
                        targetIds.Add(item.VillageId);
                }
            }
            ...
            VillageBanquetSettings source = s.GetVillageSettings(sourceItem.VillageId);

            int count = 0;
            foreach (int targetId in targetIds)
            {
                VillageBanquetSettings target = s.GetVillageSettings(targetId);
                target.EnabledGoods = new List<int>(source.EnabledGoods);
                count++;
            }
            ...
            BotLogger.Log("Banquet", BotLogLevel.Info,
                "Copied banquet settings from " + sourceItem.Display + " to " + count + " village(s).");
        }
        ...
        private class BqVillageItem
        {
            public int VillageId;
            public string Display;
            public BqVillageItem(int id, string display) { VillageId = id; Display = display; }
            public override string ToString() { return Display; }
        }
    }
}
