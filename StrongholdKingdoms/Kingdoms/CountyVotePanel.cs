...
namespace Kingdoms
{
  public class CountyVotePanel : CustomSelfDrawPanel, IDockableControl
  {
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (CountyVotePanel);
    ...
    public void init(bool resized)
    {
      ...
      this.illustrationImage.Image = (Image) GFXLibrary.parishwall_village_illlustration_02;
      ...
      if (!resized)
      {
        CountyVotePanel.StoredCountyInfo county = (CountyVotePanel.StoredCountyInfo) this.countyList[countyFromVillageId];
        bool flag = false;
        if (county == null || (DateTime.Now - county.m_lastUpdateTime).TotalMinutes > 2.0 || county.lastReturnData == null)
          flag = true;
        this.m_currentVillage = selectedMenuVillage;
        ...
        if (flag)
          return;
        this.getCountyElectionInfoCallback(county.lastReturnData);
      }
      else
        this.addPlayers();
    }
    ...
    public void getCountyElectionInfoCallback(GetCountyElectionInfo_ReturnType returnData)
    {
      ...
      if (storedCountyInfo == null)
      {
        ...
        this.countyList[returnData.countyID] = (object) storedCountyInfo;
      }
      ...
      {
        this.countyMembers.AddRange((IEnumerable<ParishMember>) returnData.countyMembers);
        int num = 0;
        foreach (ParishMember countyMember in this.countyMembers)
        {
          if (countyMember.userID == RemoteServices.Instance.UserID)
          {
            num = countyMember.numSpareVotes;
            break;
          }
        }
    ...
    public void addPlayers()
    {
      ...
      if (this.countyMembers != null)
      {
        foreach (ParishMember countyMember in this.countyMembers)
        {
          if (countyMember.userID == RemoteServices.Instance.UserID)
          {
            yourVotes = countyMember.numSpareVotes;
            break;
          }
        }
        this.countyMembers.Sort((IComparer<ParishMember>) this.parishMemberComparer);
        int position = 0;
        foreach (ParishMember countyMember in this.countyMembers)
        {
          CountyVotePanel.VoteLine control = new CountyVotePanel.VoteLine();
          if (num != 0)
            num += 5;
          control.Position = new Point(0, num);
          int numReceivedVotes = countyMember.numVotesReceived;
          if (numReceivedVotes > this.voteCap)
            numReceivedVotes = this.voteCap;
          control.init(countyMember.userName, countyMember.userID, countyMember.rank, countyMember.points, this.votingAllowed, countyMember.numSpareVotes, numReceivedVotes, countyMember.areaID, countyMember.factionID, yourVotes, position, this);
    ...
    {
      CountyVotePanel.StoredCountyInfo county = (CountyVotePanel.StoredCountyInfo) this.countyList[this.currentCounty];
      if (county != null)
        county.m_lastUpdateTime = DateTime.MinValue;
    ...
    {
      CountyVotePanel.StoredCountyInfo county = (CountyVotePanel.StoredCountyInfo) this.countyList[countyID];
      if (county == null)
        return;
      county.m_lastUpdateTime = DateTime.MinValue;
    }
