...
// Type: Kingdoms.ProvinceVotePanel
...
namespace Kingdoms
{
  public class ProvinceVotePanel : CustomSelfDrawPanel, IDockableControl
  {
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (ProvinceVotePanel);
    ...
    public void init(bool resized)
    {
      ...
      if (!resized)
      {
        ProvinceVotePanel.StoredProvinceInfo province = (ProvinceVotePanel.StoredProvinceInfo) this.provinceList[provinceFromVillageId];
        bool flag = false;
        ...
        if (flag)
          return;
        this.getProvinceElectionInfoCallback(province.lastReturnData);
      }
      else
        this.addPlayers();
    }
    ...
    public void getProvinceElectionInfoCallback(GetProvinceElectionInfo_ReturnType returnData)
    {
      ...
      {
        this.provinceMembers.AddRange((IEnumerable<ParishMember>) returnData.provinceMembers);
        int num = 0;
        foreach (ParishMember provinceMember in this.provinceMembers)
        {
          if (provinceMember.userID == RemoteServices.Instance.UserID)
          {
            num = provinceMember.numSpareVotes;
            break;
          }
        }
    ...
    public void addPlayers()
    {
      ...
      if (this.provinceMembers != null)
      {
        foreach (ParishMember provinceMember in this.provinceMembers)
        {
          if (provinceMember.userID == RemoteServices.Instance.UserID)
          {
            yourVotes = provinceMember.numSpareVotes;
            break;
          }
        }
        this.provinceMembers.Sort((IComparer<ParishMember>) this.parishMemberComparer);
        int position = 0;
        foreach (ParishMember provinceMember in this.provinceMembers)
        {
          ProvinceVotePanel.VoteLine control = new ProvinceVotePanel.VoteLine();
          if (num != 0)
            num += 5;
          control.Position = new Point(0, num);
          int numReceivedVotes = provinceMember.numVotesReceived;
          if (numReceivedVotes > this.voteCap)
            numReceivedVotes = this.voteCap;
          control.init(provinceMember.userName, provinceMember.userID, provinceMember.rank, provinceMember.points, this.votingAllowed, provinceMember.numSpareVotes, numReceivedVotes, provinceMember.areaID, provinceMember.factionID, yourVotes, position, this);
    ...
    {
      ProvinceVotePanel.StoredProvinceInfo province = (ProvinceVotePanel.StoredProvinceInfo) this.provinceList[this.currentProvince];
      if (province != null)
        province.m_lastUpdateTime = DateTime.MinValue;
