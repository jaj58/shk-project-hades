...
namespace Kingdoms
{
  public class ParishVotePanel : CustomSelfDrawPanel, IDockableControl
  {
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (ParishVotePanel);
    ...
    public void init(bool resized)
    {
      ...
      if (!resized)
      {
        ParishVotePanel.StoredParishInfo parish = (ParishVotePanel.StoredParishInfo) this.parishList[parishFromVillageId];
        bool flag = false;
        if (parish == null || (DateTime.Now - parish.m_lastUpdateTime).TotalMinutes > 2.0 || parish.lastReturnData == null)
          flag = true;
        this.m_currentVillage = selectedMenuVillage;
        ...
        if (flag)
          return;
        this.getParishMembersListCallback(parish.lastReturnData);
      }
      else
        this.addPlayers();
    }
    ...
    public void addPlayers()
    {
      ...
      if (this.parishMembers != null)
      {
        ...
        foreach (ParishMember parishMember in this.parishMembers)
        {
          ParishVotePanel.VoteLine control = new ParishVotePanel.VoteLine();
          if (num != 0)
            num += 5;
          control.Position = new Point(0, num);
          if (parishMember.numVotesReceived > this.voteCap)
          {
            int voteCap = this.voteCap;
          }
          control.init(parishMember.userName, parishMember.userID, parishMember.rank, parishMember.points, this.votingAllowed, parishMember.numSpareVotes, parishMember.numVotesReceived, parishMember.factionID, yourVotes, position, this);
    ...
    {
      ParishVotePanel.StoredParishInfo parish = (ParishVotePanel.StoredParishInfo) this.parishList[this.currentParish];
      if (parish != null)
        parish.m_lastUpdateTime = DateTime.MinValue;
